...
// Belirli bir renkte yazdırma metodu
void AnsiTerminal::printColoredAt(int row, int col, const std::string &text, const std::string &colorCode) {
    std::cout << "\033[" << row << ";" << col << "H" << colorCode << text << "\033[0m" << std::flush;
}
...
// ANSI renk kodları
const std::string AnsiTerminal::GREEN = "\033[32m";
const std::string AnsiTerminal::DEFAULT = "\033[0m";

// Renk ayarı
void AnsiTerminal::setForegroundColor(const std::string& color) {
    std::cout << color;
}

// Renk sıfırlama
void AnsiTerminal::resetColors() {
    std::cout << DEFAULT;
}
