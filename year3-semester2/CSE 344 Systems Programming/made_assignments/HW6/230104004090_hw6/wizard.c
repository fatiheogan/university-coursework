...
int main(int argc, char *argv[])
{
    ...
    /* Automatically enroll as a wizard */
    {
        char enroll[MAX_LINE];
        snprintf(enroll, sizeof(enroll), "ENROLL WIZARD %s", username);
        ...
        if (FD_ISSET(sockfd, &rset)) {
            ...
            while ((nl = memchr(start, '\n', (size_t)(rbuf + rlen - start)))) {
                *nl = '\0'; trim_crlf(start);
                ...
                if      (!strcmp(start, "OK APPARATE"))       { printf("[WIZARD %s] DISCONNECTED reason=APPARATE\n",  username); fflush(stdout); done = 1; }
                else if (!strcmp(start, "TIMEOUT DISCONNECT")) { printf("[WIZARD %s] DISCONNECTED reason=timeout\n",   username); fflush(stdout); done = 1; }
                else if (!strcmp(start, "SERVER SHUTDOWN"))    { printf("[WIZARD %s] DISCONNECTED reason=shutdown\n",  username); fflush(stdout); done = 1; }
                else if (!strncmp(start,"ERR HOGWARTS_FULL",17)){ printf("[WIZARD %s] DISCONNECTED reason=HOGWARTS_FULL\n", username); fflush(stdout); done = 1; }
                start = nl + 1;
            }
